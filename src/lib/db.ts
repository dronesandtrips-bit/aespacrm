...
// ===================== System Prompt do Robô (versionado) =====================
...
function mapBotPrompt(r: any): BotPrompt {
  return {
    id: r.id,
    version: r.version,
    title: r.title ?? "",
...
export const botPromptsDb = {
  async list(): Promise<BotPrompt[]> {
    const c = await client();
    const user_id = await uid();
    const { data, error } = await c
      .from("crm_bot_prompts")
      .select("id,version,title,content,notes,is_active,created_at,updated_at")
      .eq("user_id", user_id)
      .order("version", { ascending: false });
    if (error) throw error;
    return (data ?? []).map(mapBotPrompt);
  },

  /** Cria uma nova versão (número = maior + 1). Opcionalmente já ativa. */
  async create(input: {
    title: string;
    content: string;
    notes?: string | null;
    ...
    if (input.activate) {
      await c.from("crm_bot_prompts").update({ is_active: false }).eq("user_id", user_id).eq("is_active", true);
    }
    ...
    const { data, error } = await c
      ...
      .insert({
        user_id,
        version: nextVersion,
        title: input.title || `Versão ${nextVersion}`,
        content: input.content,
        notes: input.notes ?? null,
  ...
  async update(
    ...
    const { error } = await c.from("crm_bot_prompts").update(row).eq("id", id).eq("user_id", user_id);
    if (error) throw error;
  },

  /** Troca a versão ativa (rollback/rollforward). */
  ...
  async remove(id: string): Promise<void> {
    const c = await client();
    const user_id = await uid();
    const { error } = await c.from("crm_bot_prompts").delete().eq("id", id).eq("user_id", user_id);
    if (error) throw error;
  },
};
