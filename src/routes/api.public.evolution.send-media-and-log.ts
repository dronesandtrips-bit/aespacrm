    ...
    handlers: {
      POST: async ({ request }) => {
        try {
          ...
          // Sem messageId não conseguimos baixar a mídia depois (vira
          // "Imagem/Documento indisponível"). Nesse caso, NÃO inserimos
          ...
          if (!messageId) {
            return jsonResponse({
              ok: true,
              pending: true,
              note: `${parsed.mediatype} enviado; aguardando confirmação do WhatsApp`,
