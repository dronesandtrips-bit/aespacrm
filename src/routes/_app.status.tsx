...
function StatusPage() {
  ...
  async function addText() {
    ...
    setBusy(true);
    try {
      await api("/api/public/evolution/status-library", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "create_text", content: text.trim(), backgroundColor: "#075E54", font: 1 }) });
  ...
  return <div className="max-w-[1200px] space-y-5">
    ...
    <div className="grid md:grid-cols-2 gap-4">
      ...
      <Card className="p-4 space-y-3"><div><h2 className="font-semibold flex items-center gap-2"><Type className="size-4"/>Adicionar texto</h2><p className="text-xs text-muted-foreground">Crie uma publicação somente com texto.</p></div><Textarea value={text} onChange={(e) => setText(e.target.value)} placeholder="Digite o texto do Status" maxLength={700} rows={3}/><Button variant="outline" onClick={addText} disabled={busy || !text.trim()} className="gap-2"><Plus className="size-4"/>Adicionar à fila</Button></Card>
    </div>
    ...
    <section className="space-y-3"><div className="flex items-center justify-between"><h2 className="font-semibold">Fila de publicação ({items.length})</h2>{loading && <Loader2 className="size-4 animate-spin"/>}</div>
      {!loading && items.length === 0 ? <Card className="p-10 text-center text-muted-foreground">Adicione mídias ou textos para começar o rodízio.</Card> : <div className="grid gap-3">{items.map((item,index) => <Card key={item.id} className={`p-3 ${nextItem?.id === item.id ? "border-primary" : ""}`}><div className="flex gap-3 items-center">
        ...
        <div className="min-w-0 flex-1"><div className="flex items-center gap-2"><span className="text-xs font-semibold uppercase">{item.type}</span>{nextItem?.id === item.id && <span className="text-xs text-primary">Próximo</span>}{!item.is_active && <span className="text-xs text-muted-foreground">Pausado</span>}</div><p className="text-sm truncate mt-1">{item.content ?? item.file_name ?? "Mídia"}</p><p className="text-xs text-muted-foreground truncate">{item.caption || (item.last_used_at ? `Publicado em ${new Date(item.last_used_at).toLocaleString("pt-BR")}` : "Ainda não publicado")}</p>{item.last_error && <p className="text-xs text-destructive truncate">{item.last_error}</p>}</div>
