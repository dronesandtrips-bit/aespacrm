// Página pública /u/$token — confirmação de descadastro via link.
// Standalone (sem chrome do app, sem auth). Fluxo:
//   1. Carrega: POST /api/public/optout/info → mostra confirmação.
//   2. Usuário clica "Confirmar descadastro" → POST /api/public/optout/confirm.
//   3. Sucesso: mostra mensagem + botão "Voltar a receber" (calls /reverse).
//
// GET não muta nada — seguro contra pre-fetch de antivírus/preview.
...
function OptOutPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = await fetch("/api/public/optout/info", {
          method: "POST",
  ...
  async function handleConfirm() {
    ...
    try {
      const r = await fetch("/api/public/optout/confirm", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ token }),
      });
  ...
  async function handleReverse() {
    ...
    try {
      const r = await fetch("/api/public/optout/reverse", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ token }),
      });
