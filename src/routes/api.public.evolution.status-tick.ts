            ...
            try {
              ...
              if (completed) {
                ...
                await Promise.all([
                  ...
                  sb.from("crm_status_publications").insert({ user_id: config.user_id, media_id: run.media_id, status: "accepted", provider_response: { runId: run.id, recipients: recipients.length } }),
                ]);
              }
